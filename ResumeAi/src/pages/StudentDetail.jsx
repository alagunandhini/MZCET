...
const ScorePill = ({ label, value }) => (
  <div className="flex flex-col items-center bg-white border border-violet-100 rounded-xl px-3 py-2 min-w-[84px]">
...
const StudentDetail = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAF9FE] flex items-center justify-center text-slate-400 text-sm">
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#FAF9FE] flex items-center justify-center text-rose-500 text-sm">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FAF9FE]" style={{ fontFamily: "'Inter', ui-sans-serif, system-ui" }}>
      ...
      <div className="bg-gradient-to-r from-violet-700 via-violet-600 to-fuchsia-600">
        <div className="mx-auto max-w-5xl px-6 md:px-10 py-7">
          <button
            ...
            className="flex items-center gap-1.5 text-violet-100 text-sm font-semibold mb-3 hover:text-white transition-colors"
          >
      ...
      <div className="mx-auto max-w-5xl px-6 md:px-10 -mt-6 pb-10">
        {/* Student header card */}
        <div className="bg-white rounded-2xl border border-violet-100 shadow-[0_2px_14px_rgba(109,40,217,0.06)] p-6 mb-6 flex flex-wrap items-center gap-x-8 gap-y-3">
          <div>
        ...
        <div className="space-y-4">
          {ROUND_KEYS.map((roundKey) => {
            ...
            return (
              <div
                key={roundKey}
                className="bg-white rounded-2xl border border-violet-100 shadow-[0_2px_10px_rgba(109,40,217,0.06)] overflow-hidden"
              >
                <button
                  onClick={() => toggleRound(roundKey)}
                  className="w-full flex flex-wrap items-center justify-between gap-3 px-6 py-4 text-left hover:bg-violet-50/50 transition-colors"
                >
                ...
                {isOpen && (
                  <div className="border-t border-violet-50 px-6 py-4 space-y-3 bg-violet-50/20">
                    {round.attempts.length === 0 ? (
                      ...
                      round.attempts.map((attempt) => {
                        ...
                        return (
                          <div key={attemptKey} className="border border-violet-100 rounded-xl overflow-hidden bg-white">
                            <button
                              onClick={() => toggleAttempt(attemptKey)}
                              className="w-full flex flex-wrap items-center justify-between gap-3 px-4 py-3 hover:bg-violet-50/60 transition-colors text-left"
                            >
                              <div className="flex items-center gap-3">
                                {attemptOpen ? <ChevronDown className="h-3.5 w-3.5 text-violet-400" /> : <ChevronRight className="h-3.5 w-3.5 text-violet-300" />}
                                <span className="font-semibold text-slate-700 text-sm">
                                  Attempt {attempt.attemptNumber ?? "—"}
                                </span>
                                <span
                                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                                    attemptIsPass ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-500"
                                  }`}
                                >
                            ...
                            {attemptOpen && (
                              <div className="px-4 py-4 space-y-5 border-t border-violet-50">
                                {/* Interview Details */}
                                <div className="flex flex-wrap gap-x-6 gap-y-1.5 text-xs text-slate-500">
                                  ...
                                  <span className="font-semibold text-violet-500">{attempt.performanceLabel}</span>
                                </div>
                                ...
                                <div className="bg-violet-50/60 border border-violet-100 rounded-xl p-4">
                                  <p className="text-[11px] font-bold text-violet-500 uppercase tracking-wide mb-1">Summary</p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  <div className="bg-emerald-50/60 border border-emerald-100 rounded-xl p-4">
                                    <p className="text-[11px] font-bold text-emerald-600 uppercase tracking-wide mb-2">Strengths</p>
                                ...
                                {attempt.finalSuggestions && (
                                  <div className="bg-fuchsia-50/60 border border-fuchsia-100 rounded-xl p-4">
                                    <p className="text-[11px] font-bold text-fuchsia-600 uppercase tracking-wide mb-1">Final Suggestions</p>
                                ...
                                <div className="space-y-2">
                                  ...
                                  {attempt.qaFeedback?.map((qa, i) => {
                                    ...
                                    return (
                                      <div key={qKey} className="border border-violet-100 rounded-xl overflow-hidden">
                                        <button
                                          onClick={() => toggleQuestion(qKey)}
                                          className="w-full flex flex-wrap items-center justify-between gap-2 px-4 py-2.5 text-left hover:bg-violet-50/50 transition-colors"
                                        >
                                          <div className="flex items-center gap-2 min-w-0">
                                            {qOpen ? <ChevronDown className="h-3.5 w-3.5 text-violet-400 shrink-0" /> : <ChevronRight className="h-3.5 w-3.5 text-violet-300 shrink-0" />}
                                          ...
                                          {qa.question_score !== null && qa.question_score !== undefined && (
                                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full shrink-0 ${
                                              qa.question_score >= 50 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-500"
                                            }`}>
                                              {qa.question_score}
                                            </span>
                                          )}
                                        ...
                                        {qOpen && (
                                          <div className="px-4 py-3 border-t border-violet-50 space-y-3">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                              ...
                                              <div className="bg-fuchsia-50/60 rounded-lg p-3">
                                                <span className="text-[10px] font-bold text-fuchsia-600 uppercase">AI Suggested Answer</span>
                                            ...
                                            {qa.question_feedback && (
                                              <div className="bg-violet-50/60 rounded-lg p-3">
                                                <span className="text-[10px] font-bold text-violet-500 uppercase">AI Feedback</span>
